...
import com.itunes.parser.logging.ParserStatusMessage;
import com.itunes.parser.logging.ParserStatusUpdateLogger;
...
class TrackPropertyTagHandler implements PropertyTagHandler
{
	...
	static
	{
		...
		addPropertyToPropertyMap(TrackProperty.RATING_COMPUTED);							
		addPropertyToPropertyMap(TrackProperty.ALBUM_RATING);							
		addPropertyToPropertyMap(TrackProperty.ALBUM_RATING_COMPUTED);							
		addPropertyToPropertyMap(TrackProperty.FILE_TYPE);							
		addPropertyToPropertyMap(TrackProperty.VOLUME_ADJUSTMENT);							
		addPropertyToPropertyMap(TrackProperty.START_TIME);							
		addPropertyToPropertyMap(TrackProperty.SORT_NAME);							
		addPropertyToPropertyMap(TrackProperty.SORT_ALBUM);							
		addPropertyToPropertyMap(TrackProperty.SORT_ARTIST);							
		addPropertyToPropertyMap(TrackProperty.SORT_ALBUM_ARTIST);							
		addPropertyToPropertyMap(TrackProperty.SORT_COMPOSER);							
		addPropertyToPropertyMap(TrackProperty.RELEASE_DATE);							
		addPropertyToPropertyMap(TrackProperty.PURCHASED);							
		addPropertyToPropertyMap(TrackProperty.HAS_VIDEO);							
	}
	...
	public void handlePropertyValue(Tag propertyValue)
	{
		try
		{
			...
			else if (currentPropertyHashCode == TrackProperty.RATING_COMPUTED_HASH_CODE)
			{
				currentTrack.setRatingComputed(DataTypeParser.parseBoolean(
						TrackProperty.RATING_COMPUTED, propertyValue));
			} 
			else if (currentPropertyHashCode == TrackProperty.ALBUM_RATING_HASH_CODE)
			{
				currentTrack.setAlbumRating(DataTypeParser.parseInteger(
						TrackProperty.ALBUM_RATING, propertyValue));
			} 
			else if (currentPropertyHashCode == TrackProperty.ALBUM_RATING_COMPUTED_HASH_CODE)
			{
				currentTrack.setAlbumRatingComputed(DataTypeParser.parseBoolean(
						TrackProperty.ALBUM_RATING_COMPUTED, propertyValue));
			} 
			else if (currentPropertyHashCode == TrackProperty.FILE_TYPE_HASH_CODE)
			{
				currentTrack.setFileType(DataTypeParser.parseInteger(
						TrackProperty.FILE_TYPE, propertyValue));
			} 
			else if (currentPropertyHashCode == TrackProperty.VOLUME_ADJUSTMENT_HASH_CODE)
			{
				currentTrack.setVolumeAdjustment(DataTypeParser.parseInteger(
						TrackProperty.VOLUME_ADJUSTMENT, propertyValue));
			} 
			else if (currentPropertyHashCode == TrackProperty.START_TIME_HASH_CODE)
			{
				currentTrack.setStartTime(DataTypeParser.parseInteger(
						TrackProperty.START_TIME, propertyValue));
			} 
			else if (currentPropertyHashCode == TrackProperty.SORT_NAME_HASH_CODE)
			{
				currentTrack.setSortName(DataTypeParser.parseString(
						TrackProperty.SORT_NAME, propertyValue));
			} 
			else if (currentPropertyHashCode == TrackProperty.SORT_ALBUM_HASH_CODE)
			{
				currentTrack.setSortAlbum(DataTypeParser.parseString(
						TrackProperty.SORT_ALBUM, propertyValue));
			} 
			else if (currentPropertyHashCode == TrackProperty.SORT_ARTIST_HASH_CODE)
			{
				currentTrack.setSortArtist(DataTypeParser.parseString(
						TrackProperty.SORT_ARTIST, propertyValue));
			} 
			else if (currentPropertyHashCode == TrackProperty.SORT_ALBUM_ARTIST_HASH_CODE)
			{
				currentTrack.setSortAlbumArtist(DataTypeParser.parseString(
						TrackProperty.SORT_ALBUM_ARTIST, propertyValue));
			} 
			else if (currentPropertyHashCode == TrackProperty.SORT_COMPOSER_HASH_CODE)
			{
				currentTrack.setSortComposer(DataTypeParser.parseString(
						TrackProperty.SORT_COMPOSER, propertyValue));
			} 
			else if (currentPropertyHashCode == TrackProperty.RELEASE_DATE_HASH_CODE)
			{
				currentTrack.setReleaseDate(DataTypeParser.parseDate(
						TrackProperty.RELEASE_DATE, propertyValue));
			} 
			else if (currentPropertyHashCode == TrackProperty.PURCHASED_HASH_CODE)
			{
				currentTrack.setPurchased(DataTypeParser.parseBoolean(
						TrackProperty.PURCHASED, propertyValue));
			} 
			else if (currentPropertyHashCode == TrackProperty.HAS_VIDEO_HASH_CODE)
			{
				currentTrack.setHasVideo(DataTypeParser.parseBoolean(
						TrackProperty.HAS_VIDEO, propertyValue));
			} 
			else
			{
				logger
